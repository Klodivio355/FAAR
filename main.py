...
from utils import load_checkpoint, load_pretrained, save_checkpoint, NativeScalerWithGradNormCount, auto_resume_helper, shrink_all_caps, print_current_caps, set_inferred_ranks_from_caps, clear_inferred_ranks, enforce_zero_tail_all, show_low_high_from_images_by_indices
...
from models.lora8 import mark_only_lora_as_trainable, MTLoRALinear #lora8
...
def main(config):
    ...
    if config.MASK_GEN:
        input_folder = "/scratch/grp/grv_shi/recovered/k21220263/MTLoRA/PASCAL/PASCAL_MT/JPEGImages"
        output_folder = "/scratch/grp/grv_shi/recovered/k21220263/MTLoRA/PASCAL/PASCAL_MT/masks"
        os.makedirs(output_folder, exist_ok=True)

        sam = sam_model_registry["vit_h"](checkpoint="/scratch/grp/grv_shi/recovered/k21220263/MTLoRA/sam_vit_h_4b8939.pth")
        sam.to(device='cuda')
        mask_generator = SamAutomaticMaskGenerator(sam)

        # Sorted list of jpg files
        all_images = sorted([f for f in os.listdir(input_folder) if f.lower().endswith(".jpg")])

        # Keep only those without an existing .npy mask
        image_files = [
            f for f in all_images 
            if not os.path.exists(os.path.join(output_folder, os.path.splitext(f)[0] + ".npy"))
        ]

        print(f"Found {len(image_files)} images to process out of {len(all_images)} total.")

        for img_file in tqdm(image_files, desc="Processing images with SAM"):
            try:
                # Load image
                ...
                # Build region_id_map [H, W] with region IDs
                H, W, _ = image.shape
                region_id_map = np.full((H, W), fill_value=-1, dtype=np.int32)

                for rid, m in enumerate(masks):
                    region_id_map[m["segmentation"]] = rid

                # Save numpy file
                base_name = os.path.splitext(img_file)[0]
                out_path = os.path.join(output_folder, f"{base_name}.npy")
                np.save(out_path, region_id_map)
    ...
    lora_params = sum(p.numel()
            for name, p in model.named_parameters()
                if p.requires_grad and (
                    "lora_" in name
                    or "weight_magnitude" in name
                )
            )
    trainable_backbone_params = sum(p.numel() for name, p in model.named_parameters()
                         if 'backbone' in name and  ("lora_" in name
                    or "weight_magnitude" in name))
    ...
    Trainable Backbone Params:      {trainable_backbone_params:,}
    ...
    total_lora_freq_params = 0

    # named_modules() walks the entire tree (recursive)
    for name, module in model.named_modules():
        if "lora_freqs" in name:
            # Sum only trainable parameters within this specific sub-module
            module_params = sum(p.numel() for p in module.parameters() if p.requires_grad)
            total_lora_freq_params += module_params
            print(f"Located: {name}")
            print(f"Trainable Params in this block: {module_params:,}")
    #breakpoint()

    epoch = 0
    WARMUP_EPOCHS=0
    dropped_rank1 = print_current_caps(model)
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        if epoch >= WARMUP_EPOCHS:
            shrink_all_caps(model, margin=1, floor=2)

        print('lets print the new ranks')
        dropped_rank1 = print_current_caps(model)
        remaining_lora = lora_params - dropped_rank1
        print('How many parameters have been dropped?')
        print(dropped_rank1)
        print('New total parameter count')
        print(trainable_params - dropped_rank1)
        print('Original Backbone parameter count')
        print(trainable_backbone_params)
        print('new Backbone parameter count')
        print(trainable_backbone_params - dropped_rank1)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, task=None, teacher=None):
    ...
    #print_lora_ranks(model, title=f"Epoch {epoch} — learned ranks")
    for idx, batch in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            outputs, reg_loss = model(samples, masks=masks, training=True)
        ...
        grad_norm = loss_scaler(loss, optimizer, clip_grad=config.TRAIN.CLIP_GRAD,
                                ...
                                update_grad=(idx + 1) % config.TRAIN.ACCUMULATION_STEPS == 0, model=model, loss_dict=loss_dict)
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'reg_loss {reg_loss_meter.val:.4f} ({reg_loss_meter.avg:.4f})\t'
...
def validate(config, data_loader, model, epoch):
    ...
    for i, batch in enumerate(data_loader):
        ...
        show_low_high_from_images_by_indices(
            image_batch=images,                # [32,3,448,448]
            indices=[3, 7, 12, 25],
            save_path="freq_low_high_dom.png",
            low_band=(0.002, 1/64),           # ~64px coarse structures
            high_band=(0.25, 0.50),
            thr_type="percentile",            # or "absolute"
            low_thr=95,                       # stricter -> bigger, fewer regions
            high_thr=87,
            use_dominance=True                # key to make “low” read as smooth blobs
        )
